...
def _convert_checkpoint_for_6channel(checkpoint, in_channels=6):
    """将3通道checkpoint转换为6通道"""
    state_dict = checkpoint['model'] if isinstance(checkpoint, dict) and 'model' in checkpoint else checkpoint

    # 查找所有需要转换的第一层卷积权重
    keys_to_convert = []
    for key in state_dict.keys():
        if 'patch_embeddings.projection.weight' in key or 'convolution.weight' in key:
            weight = state_dict[key]
            # 检查是否为3通道权重但模型期望6通道
            if weight.shape[1] == 3 and in_channels == 6:
                keys_to_convert.append(key)

    # 转换权重
    for key in keys_to_convert:
        original_weight = state_dict[key]  # (out_ch, 3, kh, kw)

        # 复制权重并缩放
        expanded_weight = original_weight.repeat(1, 2, 1, 1) / 2.0  # (out_ch, 6, kh, kw)
        state_dict[key] = expanded_weight

        print(f"✓ 已转换权重 {key}: {original_weight.shape} -> {expanded_weight.shape}")
        ...
        if args.pretrain_weights is not None:
            ...
            if checkpoint:
                ...
                for name, module in self.model.named_modules():
                    if hasattr(module, 'in_channels') and module.in_channels == 6:
                        is_6channel = True
                        break

                # 如果模型是6通道但checkpoint是3通道，进行转换
                if is_6channel:
                    checkpoint_converted = _convert_checkpoint_for_6channel(checkpoint, in_channels=6)
                    self.model.load_state_dict(checkpoint_converted['model'], strict=False)
                else:
                    self.model.load_state_dict(checkpoint['model'], strict=False)
