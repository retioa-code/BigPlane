...
void Detector::ReadPlatformDetectionConfig() {
...
void Detector::Initialize(QString dom_path, QString dem_path, QString work_dir) {
    ReadPlatformDetectionConfig();
...
void Detector::Initialize(QString guaranteed_data_dir, QString work_dir) {
    ReadPlatformDetectionConfig();
...
void Detector::SetParamDebugging(QString work_dir, string id, Mat &realtime_rgb_image, Mat &padding_reference_image, int &clip_left, int &clip_top, StdVector3 &position, StdVector3 &orientation) {
    // 实时图
...
void Detector::Detect(Mat& realtime_image, StdVector3 position, StdVector3 orientation) {
